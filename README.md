...
- **Multiple retrieval strategies depending on the question:**
  ...
  | Anything else | Wide similarity search plus a full repo map (files, functions, classes) as background context |
...
- **Filename disambiguation.** If a repo has two files with the same name (e.g. two README.md files), the one closest to the repo root is preferred, since that's almost always the "main" one being asked about.
...
- **Handles messy input.** Pasting a GitHub folder/file link (a /tree/branch/... or /blob/branch/... URL) instead of the repo root URL is automatically corrected before cloning.
